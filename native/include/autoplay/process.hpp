...
public:
    ...
    template <typename T> bool read(std::uintptr_t address, T& out) const { return read(address, &out, sizeof(T)); }
    template <typename T> bool write(std::uintptr_t address, const T& value) const { return write(address, &value, sizeof(T)); }
private:
...
struct MemoryRegion { std::uintptr_t base{0}; std::size_t size{0}; std::uint32_t protection{0}; std::uint32_t state{0}; std::uint32_t type{0}; };
